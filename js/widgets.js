...
function table(headers, rows, { empty = "Nothing in this window." } = {}) {
  if (!rows.length) return `<div class="empty">${escapeHtml(empty)}</div>`;
  return `
    <div class="table-wrap">
      <table>
        <thead><tr>${headers.map((h) =>
          `<th${h.num ? ' class="num"' : ""}>${h.label}${h.hint ? hint(h.hint) : ""}</th>`).join("")}</tr></thead>
...
export const WIDGETS = [
  // ── Headline ──────────────────────────────────────────────────────
  {
    id: "total-revenue", title: "Total revenue", group: "Headline", size: "kpi",
    render: (c) => kpiHtml(term("Total revenue"), fmtCurrency(c.revenue),
      `${fmtCurrency(c.amzT.revenue)} Amazon · ${fmtCurrency(c.shopT.net)} Shopify`),
  },
  {
    id: "net-profit", title: "Net profit", group: "Headline", size: "kpi",
    render: (c) => kpiHtml("Net profit", fmtCurrency(c.pl.net),
      c.pl.net < 0 ? "after all costs and ad spend — a loss" : "after all costs and ad spend",
      c.pl.net < 0 ? "bad" : "good"),
  },
  {
    id: "contribution", title: "Contribution margin", group: "Headline", size: "kpi",
    render: (c) => kpiHtml(term("Contribution", "CONTRIBUTION MARGIN"), fmtCurrency(c.pl.contribution),
      c.revenue > 0 ? `${fmtPercent(c.pl.contribution / c.revenue)} of revenue` : "—"),
  },
  {
    id: "pl-breakdown", title: "Profit & loss breakdown", group: "Headline", size: "full",
    render: (c) => {
      const p = c.pl;
  ...
  {
    id: "revenue-vs-spend", title: "Revenue vs ad spend", group: "Headline", size: "full",
    render: () => card("Revenue vs ad spend", `
    ...
    draw: (el, c) => {
      ...
      renderDualLine(svg, c.daily, {
        height: 240, primaryKey: "spend", secondaryKey: "revenue",
  ...
  // ── Amazon ────────────────────────────────────────────────────────
  {
    id: "amz-revenue", title: "Amazon revenue", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml(term("Ordered sales", "ORDERED PRODUCT SALES"), fmtCurrency(c.amzT.revenue),
      `${fmtNumber(c.amzT.units)} units`),
  },
  {
    id: "amz-units", title: "Amazon units", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml("Units ordered", fmtNumber(c.amzT.units), `${fmtNumber(c.amzT.orderItems)} order items`),
  },
  {
    id: "amz-sessions", title: "Sessions", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml(term("Sessions"), fmtNumber(c.amzT.sessions), `${fmtNumber(c.amzT.pageViews)} page views`),
  },
  {
    id: "amz-cvr", title: "Conversion rate", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml(term("Conversion", "CVR"),
      c.amzT.sessions > 0 ? fmtPercent(c.amzT.units / c.amzT.sessions) : dash, "units ÷ sessions"),
  },
  {
    id: "amz-aov", title: "Average order value", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml(term("AOV"),
      c.amzT.orderItems > 0 ? fmtCurrency(c.amzT.revenue / c.amzT.orderItems) : dash,
      `${fmtNumber(c.amzT.orderItems)} order items`),
  },
  {
    id: "amz-refunds", title: "Refund rate", group: "Amazon", size: "kpi",
    render: (c) => kpiHtml("Refund rate",
      c.amzT.units > 0 ? fmtPercent(c.amzT.refunds / c.amzT.units) : dash,
      `${fmtNumber(c.amzT.refunds)} units returned`),
  },
  {
    id: "amz-top-skus", title: "Top SKUs by revenue", group: "Amazon", size: "half",
    render: (c) => {
      const rows = [...c.bySku].sort((a, b) => b.revenue - a.revenue).filter((s) => s.units > 0).slice(0, 8)
        .map((s) => `<tr>
      ...
      return card("Top SKUs", table(
        [{ label: "SKU" }, { label: "Units", num: true }, { label: "Revenue", num: true }, { label: "CVR", num: true, hint: "CVR" }],
        rows, { empty: "No units sold in this window." }), { flush: true, foot: "by revenue" });
    },
  },
  {
    id: "amz-sales-log", title: "What sold, and when", group: "Amazon", size: "full",
    render: (c) => {
      const rows = c.salesLog.slice(0, 40).map((r) => `<tr>
  ...
  // ── Shopify ───────────────────────────────────────────────────────
  {
    id: "shop-net", title: "Shopify net sales", group: "Shopify", size: "kpi",
    render: (c) => kpiHtml(term("Shopify net", "NET SALES"), fmtCurrency(c.shopT.net),
      `${fmtCurrency(c.shopT.gross)} gross`),
  },
  {
    id: "shop-orders", title: "Shopify orders", group: "Shopify", size: "kpi",
    render: (c) => kpiHtml("Shopify orders", fmtNumber(c.shopT.orders), `${fmtNumber(c.shopT.units)} units`),
  },
  {
    id: "shop-refunds", title: "Shopify refunds", group: "Shopify", size: "kpi",
    render: (c) => kpiHtml("Refunded", c.shopT.refunds > 0 ? fmtCurrency(c.shopT.refunds) : "$0.00",
      c.shopT.refunds > 0 ? "reversed after the sale" : "no refunds", c.shopT.refunds > 0 ? "bad" : ""),
  },
  {
    id: "shop-top-products", title: "Top Shopify products", group: "Shopify", size: "half",
    render: (c) => {
      const rows = c.byProduct.slice(0, 8).map((p) => `<tr>
      ...
      return card("Top Shopify products", table(
        ...
        rows, { empty: "No storefront orders in this window." }), { flush: true, foot: "by net of refunds" });
    },
  },

  // ── Advertising ───────────────────────────────────────────────────
  {
    id: "ad-spend", title: "Ad spend", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml("Ad spend", fmtCurrency(c.adT.cost),
      `${fmtNumber(c.adT.clicks)} clicks · ${fmtNumber(c.adT.impressions)} impressions`),
  },
  {
    id: "ad-acos", title: "ACOS", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml(term("ACOS"), c.adM.acos == null ? dash : fmtPercent(c.adM.acos),
      c.adM.roas == null ? "no attributed sales" : `${fmtRatio(c.adM.roas)} ROAS`, acosTone(c.adM.acos)),
  },
  {
    id: "ad-tacos", title: "TACOS", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml(term("TACOS"),
      c.revenue > 0 ? fmtPercent(c.adT.cost / c.revenue) : dash, "ad spend ÷ total revenue"),
  },
  {
    id: "ad-roas", title: "ROAS", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml(term("ROAS"), c.adM.roas == null ? dash : fmtRatio(c.adM.roas),
      `${fmtCurrency(c.adT.sales)} attributed`, roasTone(c.adM.roas)),
  },
  {
    id: "ad-ctr", title: "CTR", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml(term("CTR"), fmtPercent(c.adM.ctr), `${fmtCurrency(c.adM.cpc)} CPC`),
  },
  {
    id: "ad-cpa", title: "Cost per acquisition", group: "Advertising", size: "kpi",
    render: (c) => kpiHtml(term("CPA"), c.adM.cpa == null ? dash : fmtCurrency(c.adM.cpa),
      `${fmtNumber(c.adT.orders)} attributed orders`),
  },
  {
    id: "ad-by-platform", title: "Spend by platform", group: "Advertising", size: "half",
    render: (c) => {
      ...
      return card("Spend by platform", table(
        ...
        rows, { empty: "No ad rows synced." }), { flush: true });
    },
  },
  {
    id: "ad-attribution", title: "What campaigns drove sales", group: "Advertising", size: "full",
    render: (c) => {
      const rows = c.campaigns.slice(0, 12).map((r) => `<tr>
      ...
      return card("What campaigns drove sales", table(
        [{ label: "Campaign" }, { label: "Spend", num: true }, { label: "Attributed sales", num: true },
         { label: "Orders", num: true }, { label: "ACOS", num: true, hint: "ACOS" }, { label: "Clicks", num: true }],
        rows, { empty: "No campaign data in this window." }), { flush: true, foot: "by spend" });
    },
  },

  // ── Inventory ─────────────────────────────────────────────────────
  {
    id: "inv-at-cost", title: "Inventory at cost", group: "Inventory", size: "kpi",
    render: (c) => kpiHtml("Inventory at cost", fmtCurrency(c.invT.atCost),
      `${fmtNumber(c.invT.units)} units on hand`),
  },
  {
    id: "inv-oos", title: "Out of stock", group: "Inventory", size: "kpi",
    render: (c) => kpiHtml("Out of stock", fmtNumber(c.invT.oos),
      `of ${fmtNumber(c.invT.tracked)} tracked SKUs`, c.invT.oos > 0 ? "bad" : "good"),
  },
  {
    id: "inv-fba-units", title: "Units in FBA", group: "Inventory", size: "kpi",
    render: (c) => kpiHtml(term("FBA") + " units", fmtNumber(c.invT.fba),
      `${fmtNumber(c.invT.warehouse)} in warehouse`),
  },
  {
    id: "inv-low-stock", title: "Low stock", group: "Inventory", size: "half",
    render: (c) => {
      ...
      return card("Low stock", table(
        [{ label: "SKU" }, { label: "On hand", num: true }, { label: "Reorder at", num: true }, { label: "" }],
        rows, { empty: "Nothing below its reorder level." }), { flush: true });
    },
  },

  // ── Margins ───────────────────────────────────────────────────────
  {
    id: "margin-blended", title: "Blended net margin", group: "Margins", size: "kpi",
    render: (c) => kpiHtml("Blended net margin",
      c.revenue > 0 ? fmtPercent(c.pl.contribution / c.revenue) : dash,
      "after fees, COGS and shipping"),
  },
  {
    id: "margin-by-sku", title: "Margin by SKU", group: "Margins", size: "half",
    render: (c) => {
      const rows = c.invRows.filter((r) => r.cogs > 0 && r.retail > 0)
        .map((r) => ({ ...r, net: r.retail - r.cogs - r.amazonFee - r.shipCost }))
        .sort((a, b) => b.net / b.retail - a.net / a.retail).slice(0, 10)
        .map((r) => `<tr>
          <td><span class="sku-cell">${escapeHtml(r.sku)}</span></td>
      ...
      return card("Margin by SKU", table(
        ...
        rows, { empty: "No SKU has a cost on file." }), { flush: true, foot: "net of fees, COGS, shipping" });
    },
  },

  // ── Operations ────────────────────────────────────────────────────
  {
    id: "sync-status", title: "Sync status", group: "Operations", size: "half",
    render: (c) => {
      ...
      return card("Sync status", table(
        [{ label: "Job" }, { label: "Status" }, { label: "Rows", num: true }, { label: "Last run" }],
        rows), { flush: true });
    },
  },
  {
    id: "data-window", title: "Data window", group: "Operations", size: "kpi",
    render: (c) => kpiHtml("Days of data", fmtNumber(c.daily.length),
